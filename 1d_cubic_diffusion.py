...
class Model(object):
    ...
    def computeAb(self, gauss):
        ...
        A = torch.zeros([self.t_number, self.drift_order + self.diffusion_order + 2])
        ...
        for jb in range(self.drift_order + 1):
            # print("gauss1[:, :, %s]" % kd, gauss1[:, :, kd].size())
            H = torch.mean(gauss1[:, :, 0] * TX[:, :, 0] ** jb, dim=1)
            A[:, jb] = H

        # compute A by F_lkj
        for ld in range(self.diffusion_order + 1):
            F = torch.mean(gauss2[:, :, 0, 0] * TX[:, :, 0] **ld, dim=1)
            A[:, self.drift_order + 1 + ld] = F
    ...
    @utils.timing
    def compile(self, drift_order, diffusion_order, gauss_variance, type, drift_term, diffusion_term, gauss_samp_way, lhs_ratio):
    ...
    def train(self, gauss_samp_number, lam, STRidge_threshold):
        ...
        def f(X):
            return [X[0] ** 2 - self.zeta.squeeze()[self.drift_order + 1] * 2,
                    2 * X[0] * X[1] - self.zeta.squeeze()[self.drift_order + 2] * 2,
                    X[1] ** 2 - self.zeta.squeeze()[self.drift_order + 3] * 2]
        print("Diffusion: ", leastsq(f, np.array([0, 0])))
...
if __name__ == '__main__':
    ...
    samples = 10000
    dataset = DataSet(t, dt=dt, samples_num=samples, dim=1, drift_term=drift, diffusion_term=diffusion,
    ...
    model.compile(drift_order=3, diffusion_order=2, gauss_variance=1, type='LMM_2_nonequal',
                  drift_term=drift, diffusion_term=diffusion, gauss_samp_way='lhs', lhs_ratio=1.4)
