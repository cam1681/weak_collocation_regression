    ...
    def computeAb(self, gauss):
        ...
        for kd in range(self.dimension):
            for jb in range(self.basis_number):
                # print("gauss1:", gauss1.device, "self.basis:", self.basis.device, "data:", self.data.device)
...
if __name__ == '__main__':
    ...
    sample, dim = [20000, 40000], 2
    dataset = DataSet(t, true_dt=true_dt, samples_num=sample, dim=dim, drift_term=drift, diffusion_term=diffusion,
                      initialization=torch.rand(40000, dim) + 1,   # torch.normal(mean=0., std=0.1, size=(sample, dim)),
                      drift_independence=False, explosion_prevention=False, trajectory_information=False)
