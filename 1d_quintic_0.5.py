...
if __name__ == '__main__':
    ...
    samples = 5000
    dataset = DataSet(t, dt=dt, true_dt=true_dt, samples_num=samples, dim=1, drift_term=drift, diffusion_term=diffusion,
                      initialization=torch.rand(samples, 1) + 1, explosion_prevention=False)
    ...
    model.compile(basis_order=5, gauss_variance=1, type='LMM_3', drift_term=drift, diffusion_term=diffusion,
                  gauss_samp_way='lhs', lhs_ratio=0.75)
    model.train(gauss_samp_number=200, lam=0.0, STRidge_threshold=0.1)
