...
class Model(object):
    ...
    @utils.timing
    @torch.no_grad()
    def train(self, gauss_samp_number, lam, STRidge_threshold, only_13=None):
...
if __name__ == '__main__':
    ...
    dataset = DataSet(t, true_dt=true_dt, samples_num=sample, dim=dim, drift_term=drift, diffusion_term=diffusion,
                      initialization=torch.rand(sample, dim) + 1, drift_independence=True, explosion_prevention=False)
    ...
    model.compile(basis_order=3, gauss_variance=1, type='LMM_6', drift_term=drift, diffusion_term=diffusion,
                  drift_independence=True, diffusion_independence=True)
    # For 1 dimension, "drift_independence" and "diffusion_independence"  make no difference
    model.train(gauss_samp_number=1000, lam=0.0, STRidge_threshold=0.04, only_13="5D")
