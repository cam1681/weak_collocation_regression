...
class Model(object):
    ...
    def computeAb(self, gauss):
        ...
        if self.type == 'non-equal3':
            ...
            ht = torch.from_numpy(np.diff(self.t.cpu().numpy()))
...
if __name__ == '__main__':
    ...
    T, dt = 10, 0.001
    t = torch.tensor([0.1, 0.2, 0.3, 0.5, 0.7, 0.9, 1])
    ...
    drift = torch.tensor([0, 1, 0, -1]).repeat(4, 1)
    ...
    model.compile(basis_order=3, gauss_variance=1, type='non-equal3', drift_term=drift, diffusion_term=diffusion,
                  drift_independence=True, diffusion_independence=True, gauss_samp_way='lhs', lhs_ratio=3)
    # For 1 dimension, "drift_independence" and "diffusion_independence"  make no difference
    model.train(gauss_samp_number=1000, lam=0.0, STRidge_threshold=0.1, only_hat_13=False)
