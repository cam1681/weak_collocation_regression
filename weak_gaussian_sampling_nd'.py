...
approximation: drift = span{1, x, x^2,...} -- dependent !!! diffusion = diag{sigma1, sigma2, ...}
@author pi square 
...
class Model(object):
    ...
    @utils.timing
    @torch.no_grad()
    def train(self, gauss_samp_number, STRidge_threshold):
        ...
        # drift, diffusion = [self.zeta[0].numpy()], [self.zeta[self.basis_number].numpy()]
        # for i in range(self.basis_number-1):
        #     drift.extend([" + ", self.zeta[i+1].numpy(), 'x^', i+1])
        #     diffusion.extend([" + ", self.zeta[i+1+self.basis_number].numpy(), 'x^', i + 1])
        # print("Drift term: ", "".join([str(_) for _ in drift]))
        # print("Diffusion term: ", "".join([str(_) for _ in diffusion]))
        # loss = nn.MSELoss()
        # print("MSE error: ", loss(self.zeta.squeeze(), torch.cat((self.drift, self.diffusion/2))))
        

if __name__ == '__main__':
    torch.manual_seed(100)

    T, dt, true_dt = 10, 0.01, 0.02
    ...
    # dataset = DataSet(t, true_dt=true_dt, samples_num=1000, dim=1, drift_term=drift, diffusion_term=diffusion)
    ...
    model.compile(basis_order=1, gauss_variance=1, type='LMM_2', drift_term=drift, diffusion_term=diffusion)
    model.train(gauss_samp_number=200, STRidge_threshold=0.1)
