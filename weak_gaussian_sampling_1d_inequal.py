...
class Model(object):
    ...
    def computeAb(self, gauss):
        ...
        if self.type == 'adams':
            ...
            bb = torch.ones(A.size(0) - 2)
            for i in range(AA.size(0)):
                AA[i, :] = (5/12 * A[i+2, :] + 8/12 * A[i+1, :] - 1/12 * A[i, :]) * dt
                bb[i] = rb[i + 2] - rb[i + 1]
            return AA, bb
        ...
        if self.type == 'adams3':
            ...
            for i in range(AA.size(0)):
                AA[i, :] = (1/2 * (A[i+1, :] + A[i, :]) +
                        5/12 * A[i+2, :] + 8/12 * A[i+1, :] - 1/12 * A[i, :]) * dt
                bb[i] = rb[i + 2] - rb[i]
            return AA, bb

        if self.type == 'bdf2': # not stable with few time points
            AA = torch.ones(A.size(0) - 2, A.size(1))
            bb = torch.ones(A.size(0) - 2)
            for i in range(AA.size(0)):
                AA[i, :] = A[i + 2, :] * dt * 2 / 3
                bb[i] = rb[i + 2] - 4 / 3 * rb[i + 1] + 1 / 3 * rb[i] 
            return AA, bb  
        if self.type == 'non-equal-bdf2':
            AA = torch.ones(A.size(0) - 2, A.size(1))
            bb = torch.ones(A.size(0) - 2)
            ht = torch.from_numpy(np.diff(self.t.numpy()))
            print("ht: ", ht)
