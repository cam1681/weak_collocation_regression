...
if __name__ == '__main__':
    ...
    gauss_init_data = torch.normal(mean=0., std=0.1, size=(samples, 1))
    dataset = DataSet(t, dt=dt, samples_num=samples, dim=1, drift_term=drift, diffusion_term=diffusion,
                      initialization=gauss_init_data, explosion_prevention=False)
    data_true = dataset.get_data(plot_hist=False)
    print("data_true: ", data_true.shape, data_true.max(), data_true.min())

    testFunc = Gaussian
    model = Model(t, data_true, testFunc)
    ...
    model.train(gauss_samp_number=50, lam=0.00, STRidge_threshold=0.05)

    for i in range(50):
        drift = model.zeta[0:4].squeeze()
        diffusion = model.zeta[-1] if not torch.isnan(model.zeta[-1]) else torch.tensor([0])

        print("i", i, "drift", drift, "diffusion", diffusion)
        ...
        model.compile(basis_order=3, gauss_variance=1, type='LMM_6' , drift_term=drift, diffusion_term=diffusion,
                    gauss_samp_way='lhs', lhs_ratio=1)
        model.train(gauss_samp_number=50, lam=0.0, STRidge_threshold=0.0 if i<49 else 0.05)
        print(model.zeta)
