...
from data.GenerateData_inequal_ import DataSet
import time
import utils
import scipy.io
...
class Model(object):
    ...
    def train(self, gauss_samp_number, lam, STRidge_threshold):
        ...
        self.zeta.squeeze()[self.dimension * self.basis_number:] = torch.sqrt(
            2 * self.zeta.squeeze()[self.dimension * self.basis_number:])
...
if __name__ == '__main__':
    ...
    drift = torch.tensor([0, 1, 0, -1]).repeat(3, 1)
    diffusion = torch.tensor([1, 1, 1])
    samples = 2000
    dim = 3
    dataset = DataSet(t, dt=dt, samples_num=samples, dim=dim, drift_term=drift, diffusion_term=diffusion,
                      initialization=torch.normal(mean=0., std=0.1, size=(samples, dim)), explosion_prevention=False)
    ...
    model.compile(basis_order=3, gauss_variance=1, type='non-equal3', drift_term=drift, diffusion_term=diffusion,
                  gauss_samp_way='lhs', lhs_ratio=0.7)
    model.train(gauss_samp_number=20, lam=0., STRidge_threshold=0.01)
