...
class Model(object):
    ...
    def computeAb(self, gauss):
        ...
        if self.type == 'LMM_2_nonequal':
            ...
            ht = torch.ones(A.size(0) - 1).to(self.device)
            for i in range(AA.size(0)):
                ht[i] = self.t[i+1] - self.t[i]
    ...
    def train(self, gauss_samp_number, lam, STRidge_threshold, only_hat_13=False):
        ...
        if only_hat_13:
            I = torch.tensor([1, 2, 6, 7, 8, 9, 11, 12, 16, 17, 18, 19, 20, 21]).to(device)
            self.A = self.A[:, I]
...
if __name__ == '__main__':
    ...
    # model.train(gauss_samp_number=200, lam=0.0, STRidge_threshold=0.2, only_hat_13=False)
    ...
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    dt = 0.001
    t = torch.tensor([0.1, 0.3, 0.4, 0.5, 0.6, 0.8, 0.9, 1])
    ...
    diffusion = torch.ones(2)
    samples = 10000
    dataset = DataSet(t, dt=dt, samples_num=samples, dim=2, drift_term=drift, diffusion_term=diffusion,
                      initialization=torch.normal(mean=0., std=0.1, size=(samples, 1)), explosion_prevention=False)
    data = dataset.get_data(plot_hist=False)
    print("data: ", data.shape, data.max(), data.min())

    testFunc = Gaussian
    model = Model(t, data, testFunc, device)
    # model.compile(basis_order=3, gauss_variance=1, type='LMM_2_nonequal', drift_term=drift, diffusion_term=diffusion, 
