...
class Model(object):
    ...
    def computeAb(self, gauss):
        ...
        F_number = self.dimension * self.dimension * self.basis_number if self.diffusion_independence else 1
        ...
        if self.diffusion_independence:
            for ld in range(self.dimension):
                for kd in range(self.dimension):
                    for jb in range(self.dimension):
                        F = torch.mean(gauss2[:, :, ld, kd] * self.basis[:, :, jb], dim=1)
        ...
        if self.diffusion_independence:
            ...
            for i in range(self.dimension):
                for j in range(self.dimension):
                    print("Diffusion term: ", i+1, j+1, self.zeta.squeeze()[self.dimension*self.basis_number+i*self.dimension*self.basis_number+j*self.basis_number : self.dimension*self.basis_number+i*self.dimension*self.basis_number+j*self.basis_number + self.basis_number].numpy())
        else:
            print("Diffusion term: ", np.sqrt(2*self.zeta.squeeze()[self.dimension * self.basis_number].numpy()))

        # true = torch.cat((self.drift.view(-1), torch.tensor([0.5, 0, 0, 0.5])))
        # if only_hat_13:
        #     true = true[I]
...
if __name__ == '__main__':
    ...
    # diffusion = torch.ones(10)
    sample, dim = 20000, 2
    dataset = DataSet(t, true_dt=true_dt, samples_num=sample, dim=dim, drift_term=drift, diffusion_term=diffusion,
                      initialization=torch.rand(20000, dim) + 1,   # torch.normal(mean=0., std=0.1, size=(sample, dim)),
    ...
    model.compile(basis_order=4, gauss_variance=1, type='LMM_3', drift_term=drift, diffusion_term=diffusion,
                  drift_independence=False, diffusion_independence=True, gauss_samp_way='lhs', lhs_ratio=0.6)
    # For 1 dimension, "drift_independence" and "diffusion_independence"  make no difference
    model.train(gauss_samp_number=200, lam=0.0, STRidge_threshold=0.3, only_hat_13=False)
