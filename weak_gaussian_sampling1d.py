...
from data.GenerateData import DataSet
import time
import utils
import scipy.io
...
class Model(object):
    ...
    @utils.timing
    def compile(self, basis_order, gauss_variance, type, drift_term, diffusion_term, diffusion_is_polynomial, gauss_samp_way, lhs_ratio):
        ...
        self.diffusion_is_polynomial = diffusion_is_polynomial
    ...
    def train(self, gauss_samp_number, lam, STRidge_threshold):
        ...
        drift, diffusion = [self.zeta[0].numpy()], [2*self.zeta[self.basis_number].numpy()]
        for i in range(self.basis_number-1):
            drift.extend([" + ", self.zeta[i+1].numpy(), 'x^', i+1])
            diffusion.extend([" + ", np.sqrt(2*self.zeta[i+1+self.basis_number].numpy()), 'x^', i + 1])
        print("Drift term: ", "".join([str(_) for _ in drift]))
        print("Diffusion term: ", "".join([str(_) for _ in diffusion]))
        true = torch.cat((self.drift, self.diffusion**2 / 2))
        index = torch.nonzero(true).squeeze()
        if self.diffusion_is_polynomial:
            self.zeta.squeeze()[index[-1]] = self.zeta.squeeze()[index[-1] + 1]
...
if __name__ == '__main__':
    ...
    drift = torch.tensor([0, 1, 0, -1, 0, 0])
    diffusion = torch.tensor([1, 0, 0, 0, 0, 0])
    sample = 2000
    dataset = DataSet(time_instants=t, dt=dt, true_dt=Delta_t, samples_num=sample, dim=1,
                      drift_term=drift, diffusion_term=diffusion,
                      initialization=torch.rand(sample, 1) + 1,
    ...
    testFunc = Gaussian
    model = Model(torch.linspace(0, T, int(T/Delta_t) + 1), data, testFunc)
    model.compile(basis_order=5, gauss_variance=1, type='LMM_2', drift_term=drift, diffusion_term=diffusion,
                  diffusion_is_polynomial=False, gauss_samp_way='lhs', lhs_ratio=0.7)
